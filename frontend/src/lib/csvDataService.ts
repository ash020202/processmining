...
type CaseData = {
  ...
  case_id: string;
  company: string;
  end_time: string; // You can use Date if you're converting to Date object
  material_group: string;
  on_time_delivery: boolean | string; // or boolean if you parse it as boolean
  payment_term: string;
  region: string;
...
class CSVDataService {
  ...
  async loadData(): Promise<void> {
    ...
    try {
      ...
      const eventLogResult = Papa.parse(eventLogText, {
        header: true,
        skipEmptyLines: true,
        dynamicTyping: true,
      });
      ...
      const caseAttributesResult = Papa.parse(caseAttributesText, {
        header: true,
        skipEmptyLines: true,
        dynamicTyping: true,
      });

      this.caseAttributesData = caseAttributesResult.data.map(
        (item: CaseData) => ({
          ...item,
          on_time_delivery:
            item.on_time_delivery === "True" || item.on_time_delivery === true,
        })
      );
  ...
  async getFilteredCaseAttributes(
    filters: Record<string, any>
  ): Promise<CaseAttribute[]> {
    await this.loadData();
  ...
  async getUniqueValues(
    field: keyof EventLogEntry | keyof CaseAttribute
  ): Promise<string[]> {
    await this.loadData();
  ...
  async getProcessFlowData(): Promise<{
    ...
    // Create nodes
    const nodes = Array.from(activityCounts.entries()).map(
      ([activity, count]) => {
        let type = "activity";
  ...
  async getConformanceData(): Promise<{
    ...
    this.caseAttributesData.forEach((entry) => {
      if (entry.variant !== "standard_path") {
        variantCounts.set(
          entry.variant,
          (variantCounts.get(entry.variant) || 0) + 1
        );
      }
    });
  ...
  async getPerformanceMetrics(): Promise<{
    ...
    const durations = this.caseAttributesData
      .map((entry) => entry.case_duration)
      .sort((a, b) => a - b);
    const onTimeDeliveries = this.caseAttributesData.filter(
      (entry) => entry.on_time_delivery
    ).length;
    ...
    const onTimeDeliveryRate = Math.round(
      (onTimeDeliveries / this.caseAttributesData.length) * 100
    );

    // Count completed vs active cases (assuming cases without end_time are active)
    const completedCases = this.caseAttributesData.filter(
    ...
    const bottlenecks = Array.from(activityDurations.entries())
      .map(([activity, durations]) => {
        ...
        const impactScore = Math.round(
          durationFactor * 0.7 + frequencyFactor * 0.3
        );
