"use client";

import React, { useState, useEffect } from "react";

import Loader from "@/components/Loader";
import ProcessVariants from "@/components/ProcessVariants";
import RecentCases from "@/components/RecentCases";
import { transformFlowData } from "@/lib/utils";
import { Briefcase, Clock, FileCheck, Truck } from "lucide-react";
import csvDataService from "@/lib/csvDataService";
import DashboardLayout from "@/components/DashboardLayout";
import FilterBar from "@/components/FilterBar";
import KPICard from "@/components/KPICard";
import ProcessFlow from "@/components/ProcessFlow";
import Tabs from "@/components/Tabs";
import { processFlowDataType } from "@/lib/types";

// Import or define the types from ProcessFlow component
// type ProcessFlowNode = {
//   id: string;
//   label: string;
//   count: number;
//   isConformant?: boolean;
// };

// type ProcessFlowEdge = {
//   source: string;
//   target: string;
//   label?: string;
//   value?: number;
// };

// interface ProcessFlowData {
//   nodes: ProcessFlowNode[];
//   edges: ProcessFlowEdge[];
// }

interface PerformanceMetrics {
  avgLeadTime: number;
  medianLeadTime: number;
  onTimeDeliveryRate: number;
  caseCount: number;
  completedCases: number;
  activeCases: number;
}

interface Filters {
  [key: string]: string | string[] | undefined;
}

export default function Dashboard() {
  const [materialGroups, setMaterialGroups] = useState<string[]>([]);
  const [companies, setCompanies] = useState<string[]>([]);
  const [regions, setRegions] = useState<string[]>([]);
  const [filters, setFilters] = useState<Filters>({});
  const [performanceMetrics, setPerformanceMetrics] =
    useState<PerformanceMetrics>({
      avgLeadTime: 0,
      medianLeadTime: 0,
      onTimeDeliveryRate: 0,
      caseCount: 0,
      completedCases: 0,
      activeCases: 0,
    });
  const [processFlowData, setProcessFlowData] = useState<processFlowDataType>();
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [currentTab, setCurrentTab] = useState<string>("recent-cases");

  useEffect(() => {
    const loadData = async (): Promise<void> => {
      try {
        setIsLoading(true);

        // Load filter options
        const materialGroupsData = await csvDataService.getMaterialGroups();
        const companiesData = await csvDataService.getCompanies();
        const regionsData = await csvDataService.getRegions();

        setMaterialGroups(materialGroupsData);
        setCompanies(companiesData);
        setRegions(regionsData);

        // Load performance metrics
        const metrics = await csvDataService.getPerformanceMetrics();

        setPerformanceMetrics(metrics);

        // Load process flow data
        const flowData = await csvDataService.getProcessFlowData();
        // console.log(flowData);
        const { nodes, edges } = transformFlowData(
          flowData.nodes,
          flowData.edges
        );
        setProcessFlowData({ nodes, edges });

        setIsLoading(false);
      } catch (error) {
        console.error("Error loading dashboard data:", error);
        setIsLoading(false);
      }
    };

    loadData();
  }, []);

  console.log(processFlowData);

  const kpiData = [
    {
      title: "Average Lead Time",
      value: `${performanceMetrics.avgLeadTime} days`,
      trend: -2.3,
      trendLabel: "vs. last month",
      trendDirection: "down" as const,
      trendIsPositive: true,
      icon: <Clock size={20} />,
    },
    {
      title: "On-Time Delivery",
      value: `${performanceMetrics.onTimeDeliveryRate}%`,
      trend: 3.5,
      trendLabel: "vs. last month",
      trendDirection: "up" as const,
      trendIsPositive: true,
      icon: <Truck size={20} />,
    },
    {
      title: "Total Cases",
      value: performanceMetrics.caseCount.toLocaleString(),
      trend: 8.2,
      trendLabel: "vs. last month",
      trendDirection: "up" as const,
      trendIsPositive: true,
      icon: <FileCheck size={20} />,
    },
    {
      title: "Active Cases",
      value: performanceMetrics.activeCases.toLocaleString(),
      trend: -5.1,
      trendLabel: "vs. last month",
      trendDirection: "down" as const,
      trendIsPositive: true,
      icon: <Briefcase size={20} />,
    },
  ];

  return (
    <DashboardLayout>
      <FilterBar
        onFilterChange={setFilters}
        filters={filters}
        materialGroups={materialGroups}
        companies={companies}
        regions={regions}
      />
      <div className="px-6 pb-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold mb-2"> Dashboard</h1>
          <p className="text-gray-600">
            Comprehensive analysis of order-to-cash process performance using
            real CSV data
          </p>
        </div>

        {isLoading ? (
          <div className="flex justify-center items-center h-64">
            <Loader />
          </div>
        ) : (
          <>
            {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mb-6"> */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mb-6">
              {kpiData.map((item, index) => (
                <KPICard
                  key={index}
                  title={item.title}
                  value={item.value}
                  icon={item.icon}
                  trend={item.trend}
                  trendLabel={item.trendLabel}
                  trendDirection={item.trendDirection}
                  trendIsPositive={item.trendIsPositive}
                />
              ))}
            </div>

            <div className="bg-white p-6 rounded-lg shadow mb-6">
              <h2 className="text-lg font-semibold mb-4">
                Process Flow Overview
              </h2>
              <div className="">
                <ProcessFlow
                  nodes={processFlowData?.nodes}
                  edges={processFlowData?.edges}
                />
              </div>
            </div>

            <Tabs
              tabs={[
                {
                  label: "Recent Cases",
                  content: (
                    // <div className="bg-white rounded-lg shadow">
                    //   <DataTable
                    //     headers={[
                    //       "Case ID",
                    //       "Start Date",
                    //       "End Date",
                    //       "Duration",
                    //       "Status",
                    //     ]}
                    //     rows={[
                    //       [
                    //         "aa091dbf-1dc9-45cb-8362-4c825c59ed51",
                    //         "2024-03-12",
                    //         "2024-03-27",
                    //         "15.0 days",
                    //         "Completed",
                    //       ],
                    //       [
                    //         "9a576137-cb6e-4eba-b002-4253ea420f33",
                    //         "2024-01-17",
                    //         "2024-01-23",
                    //         "6.2 days",
                    //         "Completed",
                    //       ],
                    //       [
                    //         "18a6cba8-a509-4895-9328-2d10f10f77f0",
                    //         "2024-01-14",
                    //         "2024-01-30",
                    //         "16.2 days",
                    //         "Completed",
                    //       ],
                    //       [
                    //         "f4e6cdb4-e57d-4107-a859-8bd29a60ee55",
                    //         "2024-10-28",
                    //         "2024-11-07",
                    //         "10.1 days",
                    //         "Completed",
                    //       ],
                    //       [
                    //         "a7517a13-c49d-4cce-8644-8fc8dadce7b2",
                    //         "2024-03-22",
                    //         "2024-04-01",
                    //         "10.8 days",
                    //         "Completed",
                    //       ],
                    //     ]}
                    //   />
                    // </div>
                    <RecentCases />
                  ),
                  id: "recent-cases",
                },
                {
                  label: "Process Variants",
                  content: <ProcessVariants />,
                  id: "process-variants",
                },
                {
                  label: "Performance Insights",
                  content: (
                    <div className="bg-white rounded-lg shadow p-6">
                      <div className="prose ">
                        <p>
                          Analysis of the order-to-cash process reveals several
                          key insights:
                        </p>

                        <ul className="list-disc pl-5 space-y-2 mt-2">
                          <li>
                            <span className="font-medium">
                              Lead Time Variation:
                            </span>{" "}
                            Significant variation in lead times across
                            companies, with Drystone India showing the best
                            performance at {performanceMetrics.avgLeadTime} days
                            average.
                          </li>
                          <li>
                            <span className="font-medium">
                              On-Time Delivery:
                            </span>{" "}
                            {performanceMetrics.onTimeDeliveryRate}% of cases
                            are delivered on time, with room for improvement in
                            specific regions.
                          </li>
                          <li>
                            <span className="font-medium">
                              Process Variants:
                            </span>{" "}
                            57.1% of cases follow the standard path, while 17.0%
                            involve price changes and 12.4% involve material
                            changes.
                          </li>
                          <li>
                            <span className="font-medium">Bottlenecks:</span>{" "}
                            The Clear Invoice activity has the longest duration
                            at 5.2 days on average, representing a key
                            opportunity for process improvement.
                          </li>
                        </ul>
                      </div>
                    </div>
                  ),
                  id: "performance-insights",
                },
              ]}
              activeTab={currentTab}
              onTabChange={function (tabId: string): void {
                console.log(tabId);
                setCurrentTab(tabId);
              }}
            />
          </>
        )}
      </div>
    </DashboardLayout>
  );
}
