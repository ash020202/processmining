  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const priceChangeCases = caseAttributes.filter(
          (c) => c.variant === "price_change_path"
        );
    ...
    const analyzeRootCauses = (
      ...
      const targetCases = data.filter(
        (c: { [x: string]: any }) => c[targetField] === targetValue
      );
      ...
      dimensions.forEach((dimension: string | number) => {
        ...
        const dimensionResults = uniqueValues.map((value) => {
          ...
          const rate =
            countWithValue > 0
              ? (targetCountWithValue / countWithValue) * 100
              : 0;
          ...
          return {
            ...
            contribution:
              countWithValue > 0 ? (countWithValue / totalCount) * 100 : 0,
          };
        });
      ...
      return results.sort(
        (a: { impact: number }, b: { impact: number }) =>
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Identification of root causes for process deviations and performance
          issues using real CSV data
        </p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="prose max-w-none">
              <p>
                Root cause analysis of process issues reveals several key
                insights:
              </p>

              <ul className="list-disc pl-5 space-y-2 mt-2">
                <li>
                  <span className="font-medium">Late Delivery Factors:</span>{" "}
                  The strongest predictors of late delivery are payment terms
                  (60+ days), certain material groups (Fertilizers), and
                  specific companies (Drystone India).
                </li>
                <li>
                  <span className="font-medium">Price Change Drivers:</span>{" "}
                  Price changes occur most frequently with Building Materials
                  and Safety Equipment, suggesting pricing volatility in these
                  categories.
                </li>
                <li>
                  <span className="font-medium">Regional Variations:</span> The
                  Asia-Pacific region shows significantly higher rates of
                  process deviations compared to Europe, indicating potential
                  process standardization issues.
                </li>
                <li>
                  <span className="font-medium">Company Practices:</span>{" "}
                  Companies with higher conformance rates also show better
                  on-time delivery performance, confirming the link between
                  process discipline and outcomes.
                </li>
              </ul>
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>
                  Implement special monitoring for orders with identified risk
                  factors for late delivery.
                </li>
                <li>
                  Review pricing strategies for Building Materials and Safety
                  Equipment to reduce the need for price changes.
                </li>
                <li>
                  Standardize processes across regions, with particular focus on
                  Asia-Pacific operations.
                </li>
                <li>
                  Share best practices from high-performing companies with the
