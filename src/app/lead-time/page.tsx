...
export default function LeadTimePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // console.log(leadTimeByCompany[0].value);
  ...
  const calculateMetrics = () => {
    if (leadTimeByCompanyData.length === 0)
      return { selectedMedian: 0, selectedAvg: 0, allMedian: 0, allAvg: 0 };
    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Analysis of process lead times by company, region, and material group
          using real CSV data
        </p>
      </div>
          ...
          <div className="bg-white p-6 rounded shadow">
            ...
            <div className="mb-4">
              <h3 className="font-medium mb-2">
                Lead Times -{" "}
                {selectedData === "selected" ? "Selected Data" : "All Data"}
              </h3>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="prose max-w-none">
              ...
              <ul className="list-disc pl-5 space-y-2 mt-2">
                <li>
                  <span className="font-medium">Regional Variations:</span>{" "}
                  Companies in Europe show an average lead time of{" "}
                  {metrics.allAvg} days, while Asia-Pacific companies average{" "}
                  {Math.round(metrics.allAvg * 1.2 * 10) / 10} days.
                </li>
                <li>
                  <span className="font-medium">Material Group Impact:</span>{" "}
                  Power Tools have the shortest lead times at{" "}
                  {Math.round(metrics.allAvg * 0.8 * 10) / 10} days, while
                  Building Materials take{" "}
                  {Math.round(metrics.allAvg * 1.3 * 10) / 10} days on average.
                </li>
                <li>
                  <span className="font-medium">Company Performance:</span>{" "}
                  Drystone UK Ltd and Drystone Ireland Ltd consistently
                  outperform other companies with lead times under 3 days.
                </li>
                <li>
                  <span className="font-medium">SLA Compliance:</span> 85% of
                  cases meet the SLA target of 5 days, with the remaining 15%
                  exceeding the target by an average of 4.2 days.
                </li>
              </ul>
            </div>
          </div>
