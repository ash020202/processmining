...
const ProcessFlow: React.FC<ProcessFlowProps> = ({ nodes, edges }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <div className="flex flex-col space-y-8 max-h-[500px] overflow-y-auto">
        {nodes.map((node, index) => (
          <div key={node.id} className="flex flex-col items-center">
            <div
              id={node.id}
              className={`process-node ${
                node.isConformant ? "conformant" : "non-conformant"
              } w-64`}
            >
      ...
      <div className="mt-4 text-sm text-gray-500">
        <p>
          Note: This is a simplified visualization. In the actual
          implementation, we would use a proper graph visualization library to
          show the complete process flow with all edges and connections between
          activities.
        </p>
      </div>
    </div>
